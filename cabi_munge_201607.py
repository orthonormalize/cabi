...
if (not((os.path.isfile(dbName)))): # assume that TH is done, if db exists
    TH_csv2db(134,154,dbName,tableTH)
    weather_csv2db(weatherFile,dbName,tableW)
    stations_csv2db(stationsCSV,dbName,tableS)
DF0 = rek_readSQL(dbName,tableTH)
tossStations = badStationLocs()
DF1 = DF0.loc[~((DF0['startLoc'].isin(tossStations)) | (DF0['endLoc'].isin(tossStations)))]
W = rek_readSQL(dbName,tableW)
S = rek_readSQL(dbName,tableS)
dDF = mergeData(DF1,W,S)
...
rek_writeSQL(dbName,tableD,dDF,'w')
rek_writeSQL(dbName,tableB,bDF,'w')