    ...
    try:	
        ...
        try:
            if (len(dfS)):
                ...
                try:
                    rawString = (requests.get(N['scrapeURL'])).content
                    [subject2,body2] = buildEmailAlert('raw XML',rawString,P,dbName(N,dumpCount))
                    sendGmail(N['eFailLogFile'],N['eAddr'],N['eAddr'],N['ePass'],subject2,body2)
                except:
                    pass
        except:
            pass
        DF = DF[DF['latestupdatetime']<-9999] # empty DF, but keep cols        
